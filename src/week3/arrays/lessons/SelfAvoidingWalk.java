...
public class SelfAvoidingWalk {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < trials; i++) {
            boolean[][] a = new boolean[n][n];      // intersections visited, true for visited
            int x = n/2, y = n/2;                   // current position, starts at the center of the lattice 

            // repeatedly take a random step, unless you've already escaped
            while (x > 0 && x < n-1 && y > 0 && y < n-1) {
                
                // dead end, so break the loop
